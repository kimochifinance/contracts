...
constructor(
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (forBurn > 0) {
            pool.rewardToken.mint(address(this), forBurn);
        ...
        if (forDev > 0) {
            ...
            if (teamAddresses[_pid] != address(0)) {
                //pool.rewardToken.mint(teamAddresses[_pid], forDev.sub(lockAmount));
        ...
        //pool.rewardToken.mint(address(this), forFarmer);
    ...
    function getPoolReward(uint256 _pid) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 rewardCanAlloc = pool.rewardToken.capfarm().sub(pool.rewardToken.totalSupply());
